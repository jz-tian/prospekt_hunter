...
export function parseDennsPageData(payload, weekScope = "current", now = new Date(), marketSlug = DENNS_DEFAULT_MARKET_SLUG) {
  ...
  const marketOffers = payload?.result?.data?.sanityAllOffers?.nodes ?? [];
  const nationalOffers = payload?.result?.data?.noLimitOffers?.nodes ?? [];
  const offers = [...marketOffers, ...nationalOffers];
