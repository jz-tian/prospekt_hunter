...
function seedStaticData() {
  const retailerCount = get("SELECT COUNT(*) as count FROM retailers").count;
  if (retailerCount === 0) {
    const stmt = db.prepare("INSERT OR IGNORE INTO retailers (slug, name, color) VALUES (?, ?, ?)");
    for (const retailer of RETAILERS) {
      stmt.run(retailer.slug, retailer.name, retailer.color);
    }
  }

  const categoryCount = get("SELECT COUNT(*) as count FROM categories").count;
  if (categoryCount === 0) {
    const stmt = db.prepare("INSERT OR IGNORE INTO categories (slug, name, tone, text_color, parent_id) VALUES (?, ?, ?, ?, NULL)");
    for (const category of CATEGORY_DEFINITIONS) {
      stmt.run(category.slug, category.name, category.tone, category.textColor);
    }
  }
}
  ...
  for (const [retailerSlug, fixturesByWeek] of Object.entries(SAMPLE_FIXTURES)) {
    ...
    for (const fixture of Object.values(fixturesByWeek)) {
      ...
      for (const offer of fixture.offers) {
        ...
        run(
          `INSERT INTO offers
           (prospekt_issue_id, product_name, brand, sale_price, unit_info, image_url, product_url, category_id, source_section, confidence_score, review_state)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            issueId,
            offer.productName,
...
export function getDashboardData(weekScope = "current") {
  ...
  const featuredOffers = all(
    baseOfferQuery("WHERE offers.review_state = 'approved' AND prospekt_issues.week_scope = ?", "ORDER BY offers.sale_price ASC, offers.product_name ASC LIMIT 6"),
    [selectedWeek]
  );
...
export function listOffers(filters = {}) {
  ...
  let orderClause = "ORDER BY offers.sale_price ASC, offers.product_name ASC";
  if (filters.sort === "price_asc") {
    orderClause = "ORDER BY offers.sale_price ASC";
  } else if (filters.sort === "price_desc") {
    orderClause = "ORDER BY offers.sale_price DESC";
...
export function replaceFixtureData(retailerSlug, issue, offers) {
  const retailer = get("SELECT id FROM retailers WHERE slug = ?", [retailerSlug]);
  if (!retailer) {
