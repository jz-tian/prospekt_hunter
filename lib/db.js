...
function cleanupRemovedCategories() {
  const activeSlugs = new Set(CATEGORY_DEFINITIONS.map((category) => category.slug));
  const staleCategories = all("SELECT id, slug FROM categories").filter((category) => !activeSlugs.has(category.slug));

  for (const category of staleCategories) {
    run("UPDATE offers SET category_id = NULL WHERE category_id = ?", [category.id]);
...
export function clearShoppingItems(retailerSlug = null) {
  if (retailerSlug) {
    return run(
...
export function reclassifyOffers() {
  const rows = all("SELECT id, product_name AS productName, source_section AS sourceSection FROM offers");
  ...
  try {
    for (const row of rows) {
      ...
      const category = get("SELECT id FROM categories WHERE slug = ?", [categorySlug]);
      run("UPDATE offers SET category_id = ? WHERE id = ?", [category.id, row.id]);
    }
